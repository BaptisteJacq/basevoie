...
COMMENT ON TABLE G_BASE_VOIE.TA_TRONCON IS 'Table contenant tous les tronçons de la base voie.';
...
COMMENT ON COLUMN G_BASE_VOIE.TA_TRONCON.ordre_troncon IS 'Ordre dans lequel les tronçons se positionnent afin de contituer la voie administrative. 1 est égal au début de la voie et 1 + n est égal au tronçon suivant.';
...
COMMENT ON COLUMN G_BASE_VOIE.TA_TRONCON.fid_etat IS 'Etat d''avancement des corrections : en erreur, corrigé, correct.';
COMMENT ON COLUMN G_BASE_VOIE.TA_TRONCON.fid_voie_physique IS 'Clé étrangère vers la table TA_VOIE_PHYSIQUE permettant d''associer un tronçon à une et une seule voie physique.';
...
ALTER TABLE G_BASE_VOIE.TA_TRONCON
ADD CONSTRAINT TA_TRONCON_FID_ETAT_FK
...
CREATE INDEX TA_TRONCON_FID_ETAT_IDX ON G_BASE_VOIE.TA_TRONCON(fid_etat)
    TABLESPACE G_ADT_INDX;
