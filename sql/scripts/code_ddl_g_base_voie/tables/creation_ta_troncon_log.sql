...
COMMENT ON TABLE G_BASE_VOIE.TA_TRONCON_LOG IS 'Table d''historisation des actions effectuées sur les tronçons de la base voie.';
COMMENT ON COLUMN G_BASE_VOIE.TA_TRONCON_LOG.objectid IS 'Clé primaire auto-incrémentée de la table.';
COMMENT ON COLUMN G_BASE_VOIE.TA_TRONCON_LOG.id_troncon IS 'identifiant de la table TA_TRONCON permettant de savoir sur quel tronçon ont été effectué les actions.';
COMMENT ON COLUMN G_BASE_VOIE.TA_TRONCON_LOG.id_voie_physique IS 'Identifiant de la table TA_VOIE_PHYSIQUE permettant d''associer un tronçon à une et une seule voie physique. ';
...
CREATE INDEX TA_TRONCON_LOG_FID_TRONCON_IDX ON G_BASE_VOIE.TA_TRONCON_LOG(id_troncon)
...
CREATE INDEX TA_TRONCON_LOG_ID_VOIE_IDX ON G_BASE_VOIE.TA_TRONCON_LOG(id_voie_physique)
    TABLESPACE G_ADT_INDX;
